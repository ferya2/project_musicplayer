import 'package:flutter/material.dart';

class ColorsCollection {
  static Color primaryColor = Colors.purple;
  static Color secondaryColor = Colors.purpleAccent;
  static Color blackNeutral = Colors.black;
  static Color whiteNeutral = Colors.white;
  static Color greyNeutral = Colors.grey;
  static Color redNeutral = Colors.red;
  static Color blueNeutral = Colors.blue;
  static Color bgColor = const Color(0xFF1E1E1E);
  static Color bgCircleIcon = const Color(0xFF030303);
  static Color textField = const Color(0xFF777777);
  static Color signBtn = const Color(0xFF535353);
  static Color nextBtn = const Color(0xFF000000);
  static Color greenNeutral = const Color(0xFF1ED760);
  static Color blackGrey = const Color(0xFF121212);
  static Color abuTextField2 = const Color(0xFF282828);
  static Color abuTextField = const Color(0xFF777777);
  static Color abuBtnText = const Color(0xFF535353);
  static Color abuWrap = const Color(0xFF9C9C9C);
  static Color floatingPlayerMusic = const Color(0xFF550A1C);
  static Color textSubList = const Color(0xFFB3B3B3);
  static Color libraryGreeenAccent = const Color(0xFF00667B);
  static Color libraryGreeenDark = const Color(0xFF002F38);
  static Color darkGradient = const Color(0xFF101010);
  static Color lightGrey = const Color(0xFF3E3F3F);
  static Color reqs4TextGr = const Color(0xFF1ED760); 
  static Color reqs4CreateBtn = const Color(0xFFF5F5F5);
  static Color lessGreen = const Color(0xFFF5F5F5);
  static Color transparent = Colors.transparent;
  static Color checkMarkOutline = const Color(0xFF727272);
}
